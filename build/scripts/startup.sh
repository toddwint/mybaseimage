...
# ttyd with color
nohup ttyd -p "$HTTPPORT1" -t titleFixed="${APPNAME}|${APPNAME}.log" -t fontSize=18 -t 'theme={"foreground":"black","background":"white", "selection":"red"}' /opt/"$APPNAME"/scripts/tmux-session.sh >> /opt/"$APPNAME"/logs/ttyd.nohup 2>&1 &
# ttyd without color
#nohup ttyd -p "$HTTPPORT1" -t titleFixed="${APPNAME}|${APPNAME}.log" -T xterm-mono -t fontSize=18 -t 'theme={"foreground":"black","background":"white", "selection":"red"}' /opt/"$APPNAME"/scripts/tmux-session.sh >> /opt/"$APPNAME"/logs/ttyd.nohup 2>&1 &
